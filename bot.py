    ...
    for admin_id in config.ADMIN_IDS:
        try:
            ...
            await ctx.bot.send_message(
                ...
                f"👤 User: [{u.full_name}](tg://user?id={u.id}) (`{u.id}`)\n"
...
async def show_pending(query, ctx):
    ...
    for o in orders:
        text = (
            f"⏳ *Pending Order*\n\n"
            f"👤 User: [{o['full_name']}](tg://user?id={o['user_id']}) (`{o['user_id']}`)\n"
            f"📋 Plan: {o['plan_name']}\n"
            f"💰 Amount: ₹{o['amount']:.0f}\n"
            f"💳 Method: {o['payment_method'] or '—'}\n"
            f"🆔 Order: `{o['order_id']}`\n"
...
async def show_users(query):
    ...
    for u in users[:20]:
        ...
        text += f"• `{u['user_id']}` {u['full_name']} {uname}\n"
    if len(users) > 20:
...
async def show_plans(query):
    ...
    for p in plans:
        ...
        text += f"{status} *{p['name']}* – ₹{p['price']:.0f} / {p['duration']}d\n"
    kb = []
    for p in plans:
        row = [
    ...
    for s in db.get_expiring_soon(48):
        try:
            await ctx.bot.send_message(
                ...
                f"Your subscription for *{s['plan_name']}* will expire in "
    ...
    for e in db.expire_subscriptions():
        try:
            await ctx.bot.send_message(
                ...
                f"Your subscription for *{e['plan_name']}* has expired.\n\n"
